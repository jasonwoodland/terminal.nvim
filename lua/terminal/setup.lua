...
function M.setup_keymap(api)
	...
	if keys.digraph ~= false then
		local _digraph_ns = vim.api.nvim_create_namespace("terminal_digraph")
		local _digraph_win = nil

		vim.keymap.set("t", keys.digraph, function()
			local job = vim.b.terminal_job_id
			if not job then return end

			-- Cancel any in-progress digraph session
			vim.on_key(nil, _digraph_ns)
			if _digraph_win and vim.api.nvim_win_is_valid(_digraph_win) then
				vim.api.nvim_win_close(_digraph_win, true)
			end
			...
			local width = 11
			_digraph_win = vim.api.nvim_open_win(buf, false, {
				relative = "editor",
				width = width,
				height = 1,
				row = row,
			...
			local function show(text)
				if not (_digraph_win and vim.api.nvim_win_is_valid(_digraph_win)) then return end
